    ...
    for _, module_name, ispkg in pkgutil.iter_modules([str(package_path)]):
        ...
        try:
            ...
            for name, obj in inspect.getmembers(module, inspect.isclass):
                ...
                if (
                    issubclass(obj, ChartController)
                    and obj != ChartController
...
def run_all_controllers():
    ...
    logger.info(
        f"Found {len(controller_classes)} chart generators: {', '.join(controller_classes.keys())}"
    )
    ...
    for class_name, controller_class in controller_classes.items():
        try:
            ...
            controller_files, _ = count_files_modified_after(
                controller.outdir if hasattr(controller, "outdir") else charts_dir, controller_start
            )
            controller_file_counts[class_name] = controller_files

            successful.append(class_name)
            logger.info(
                f"Successfully completed charts for {class_name} ({controller_files} files)"
            )
    ...
    if controller_file_counts:
        ...
        for controller, count in sorted(
            controller_file_counts.items(), key=lambda x: x[1], reverse=True
        ):
