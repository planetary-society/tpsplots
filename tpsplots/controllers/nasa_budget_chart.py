...
class NASABudgetChart(ChartController):
    ...
    def nasa_budget_by_year_with_projection_inflation_adjusted(self):
        ...
        first_projection_year = df.loc[df["White House Budget Projection"].notna(), "Fiscal Year"].min()
        if pd.isna(first_projection_year):
            first_projection_year = fiscal_years.max()  # If no projections, set to max fiscal year to avoid filtering
