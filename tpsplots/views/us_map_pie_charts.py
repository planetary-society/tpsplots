...
class USMapPieChartView(ColorCycleMixin, ChartView):
    ...
    def _create_chart(self, metadata, style, **kwargs):
        ...
        offset_positions = self._calculate_offset_positions(
    ...
    def _draw_pie_improved(
        ...
        for i, (r1, r2) in enumerate(itertools.pairwise(pie)):
            ...
            angles = np.linspace(
                2 * np.pi * r1 + start_angle_offset, 2 * np.pi * r2 + start_angle_offset, 50
            )
    ...
    def _create_fallback_map(self, ax):
        ...
        ax.add_patch(
            plt.Rectangle((-125, 20), 58.5, 30, facecolor="lightgray", alpha=0.7, zorder=0)
        )
