...
class LollipopChartView(ChartView):
    ...
    def _create_chart(self, metadata, style, **kwargs):
        ...
        if categories is None or start_values is None or end_values is None:
            raise ValueError(
                "categories, start_values, and end_values are required for lollipop_plot"
            )
        ...
        marker_size = kwargs.pop(
            "marker_size", style.get("marker_size", 8) * 2
        )  # Larger for lollipops
        ...
        for i, (_cat, start_val, end_val) in enumerate(
            zip(categories, start_values, end_values, strict=False)
        ):
            y_pos = y_positions[i]
    ...
    def _get_sort_indices(self, categories, start_values, end_values, sort_by, ascending=False):
        ...
        if sort_by == "start":
            ...
            raise ValueError(
                f"sort_by must be 'start', 'end', 'range'/'duration', or 'categories', got {sort_by}"
            )
        ...
        if specific_colors is not None:
            ...
            if isinstance(specific_colors, str):
                ...
                if len(specific_colors) < num_categories:
                    ...
                    return list(specific_colors) + [specific_colors[-1]] * (
                        num_categories - len(specific_colors)
                    )
            ...
            if isinstance(fallback_colors, str):
                ...
                if len(fallback_colors) < num_categories:
                    return list(fallback_colors) + [fallback_colors[-1]] * (
                        num_categories - len(fallback_colors)
                    )
    ...
    def _format_lollipop_chart(
        self, ax, categories, y_positions, start_values, end_values, style, **kwargs
    ):
        """Format the lollipop chart appearance."""
        # Handle category label wrapping
        category_wrap_length = kwargs.pop(
            "category_wrap_length", style.get("label_wrap_length", 20)
        )
    ...
    def _add_value_labels(
        ...
        for _i, (y_pos, start_val, end_val) in enumerate(
            zip(y_positions, start_values, end_values, strict=False)
        ):
