...
Returns:
dict with keys: - data: Full DataFrame with all columns including adjusted - Year: Series of fiscal year datetimes - {col}: Nominal series for each monetary column - {col}\_adjusted_nnsi: Adjusted series for each monetary column - {col}\_sum: int sum of nominal values - {col}\_adjusted_nnsi_sum: int sum of adjusted values - export_df: DataFrame for CSV export - metadata: dict with standard keys (min/max FY, inflation year, source)
...
Returns:
dict with keys: - data: Full DataFrame with all columns including adjusted - Fiscal Year: Series of fiscal year datetimes - {col}: Nominal series for each monetary column - {col}\_adjusted_nnsi: Adjusted series for each monetary column - {col}\_sum: int sum of nominal values - {col}\_adjusted_nnsi_sum: int sum of adjusted values - export_df: DataFrame for CSV export - metadata: dict with standard keys (min/max FY, inflation year, source)
...
Returns:
dict with keys: - data: Full DataFrame with all columns including adjusted - Fiscal Year: Series of fiscal year datetimes - Lunar effort % of NASA: Percentage series - {col}: Nominal series for each monetary column (original name) - {col}\_adjusted_nnsi: Adjusted series for each monetary column - {col}\_sum: int sum of nominal values for each monetary column - {col}\_adjusted_nnsi_sum: int sum of adjusted values - export_df: DataFrame for CSV export - metadata: dict with standard keys (min/max FY, inflation year, source)
