...
Returns:
dict with keys: - values: dict mapping category names to block counts - labels: list of formatted labels with percentages - fiscal_year: int - The fiscal year of the data - source: str - Source attribution text - block_value: int - Dollar value per block - export_df: DataFrame for CSV export
