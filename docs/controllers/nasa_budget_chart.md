...
Returns:
dict with all DataFrame columns as keys (pass-through), plus: - data: full DataFrame - export_df: DataFrame for CSV export - metadata: dict with max_fiscal_year, min_fiscal_year, source,
...
Returns:
dict with all DataFrame columns as keys (pass-through), plus: - data: full DataFrame - export_df: DataFrame for CSV export - metadata: dict with fiscal year ranges, per-column min/max/FY stats,
